...
const server = ws.createServer(function (connection) {
  ...
  connection.on("close", function () {
    //当同一个账号在同一个浏览器的多个窗口打开时，会有多个userId相同的连接，如果用filter就全部下线了，我们应该只删除当前窗口的连接
    const index = onlineList.findIndex(
      (item) => item.id === connection.user.id
    );
