 ...
 * @param {*} param
 */
const createMessage = async function (param, sessionId) {
  ...
  const sql = `insert into messages (${Object.keys(insertObj).join(
    ","
  )}) values (${Object.values(insertObj).join(",")})`;
 ...
 * 获取留言列表
 */
const getMessages = async function (query) {
  const { current = 0, pageSize = 10 } = query;
  //获取留言信息
  const sqlMsg = `select SQL_CALC_FOUND_ROWS * from messages where pid=-1 order by createTime desc limit ${
    current * pageSize
  },${pageSize}`;
  ...
  let resReply = [];
  if (pids.length) {
    const sqlReply = `select * from messages where pid in (${pids.join(
      ","
    )}) order by createTime`;
 ...
 * @param {*} sessionId
 */
const deleteMessage = async function (param, sessionId) {
  const loginName = jwt.verify(sessionId, TOKEN_SECRETKEY).username;
  const userRes = await getUser({ username: loginName });
  const user = userRes.data || {};
...
/**
 * 当更新用户名或者用户头像时，更新他的留言的用户名和头像
 * @param {*} user
 */
const updateUserMessage = function (user) {
  ...
  Promise.all([exec(sql), exec(sql2)]);
};
